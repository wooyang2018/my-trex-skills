...
name: wiki-report
description: >
  生成思源 wiki 的报告和视图，包含两种模式：（1）仪表板——在 wiki 内创建仪表板文档，
  其表格和卡片由 `query_sql` 结果渲染。当用户说"创建仪表板"、"wiki 仪表板"、
  "把所有 X 显示为表格"、"结构化视图"、"查询我的笔记本"、"构建内容索引"、
  "显示所有概念/实体/项目"时使用。输出可重建的 markdown 快照文档。（2）摘要——
  生成周期性知识摘要，一份可读的通讯风格总结，概述指定期内 wiki 中学到的、更新的
  和连接的内容。当用户说"这周我学到了什么"、"给我一个摘要"、"每周总结"、
  "知识报告"、"我的 wiki 有什么新内容"、"总结我最近的学习"时使用。
---
...
3. 读取 `/<SIYUAN_NOTEBOOK_NAME>/_meta/taxonomy` 和 `/<SIYUAN_NOTEBOOK_NAME>/index` 了解笔记本当前范围。
...
| 页面 | 标签 | 摘要 | 更新时间 |
|---|---|---|---|
| [[<page-slug>]] | tag-a, tag-b | <摘要摘录> | <YYYY-MM-DD> |
...
siyuan-sisyphus block append \
  ...
  --data-type markdown \
  --data "- [<ISO-8601>] WIKI_DASHBOARD recipe=<recipe> slug=<dashboard-slug> rows=<N>"
```
...
| 页面 | 类别 | 摘要 |
|---|---|---|
| [[concepts/foo]] | concept | frontmatter 中的一句话摘要 |

### 值得注意的更新（[计数]）
| 页面 | 变更内容 |
|---|---|
| [[skills/react-hooks]] | 添加了 useCallback 与 async effects 的模式 |
...
- [[concepts/A]] → [[entities/B]] — [简洁说明此连接为何有趣]
- [[synthesis/X]] 创建——首次桥接 [[concepts/Y]] 和 [[concepts/Z]]

## 待解线索

- **待编译草稿**（[计数]）：[[concepts/foo]], [[concepts/bar]]
...
- [[concepts/X]] — [具体原因："本周 3 篇新论文都扩展了此概念"]
- [[synthesis/Y]] — [具体原因："本周创建的 2 个新页面引用了它"]
...
```
# 仪表板
- [TIMESTAMP] WIKI_DASHBOARD recipe=<recipe> slug=<dashboard-slug> rows=<N>

# 摘要
- [TIMESTAMP] DIGEST period="<period>" new_pages=N updated_pages=M themes=T connections=C saved=<true|false>
```
