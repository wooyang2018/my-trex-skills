...
description: >
  LLM Wiki 完整宪法——三层架构、笔记本组织、特殊文档、存储原语、危险演练、
  校准说明（10 条 siyuan-sisyphus 实测行为）、环境变量、操作模式。
  本文件是 llm-wiki skill 的核心参考文档，包含所有模块共享的架构规范。
---
...
受控标签词汇表，由 lint 模块（标签分类法模式） 管理。纯 markdown 正文。
...
> **解析配置 + 预检**——遵循 `references/constitution.md` 中的配置解析协议。从 CWD 向上查找 `.env`，回退到 `~/.siyuan-wiki/config`，否则提示设置。这会得到 `SIYUAN_NOTEBOOK_ID`（用于 `--notebook` 和 `query_sql ... WHERE box=`）、`SIYUAN_NOTEBOOK_NAME`（用于 `fs *` 路径）和任何工具特定的路径覆盖。然后运行 `siyuan-sisyphus --version` 和 `notebook get-permissions --notebook "$SIYUAN_NOTEBOOK_ID"`；失败则停止。
...
- **setup** — 笔记本初始化 + 多笔记本管理（`references/setup.md`）
- **ingest** — 统一知识蒸馏入口，将源文档/URL/对话/数据蒸馏为 wiki 页面（`references/ingest/index.md`）
- **ingest-synthesis** — 综合发现（内部共现图）+ 自主研究（外部多轮调查）（`references/ingest/synthesis.md`）
- **lint** — 健康审计（13 项检查）+ Consolidate 自动修复 + 日常维护 + 图谱维护（`references/lint/index.md` 路由入口，含 10 个子模块）
- **report** — 仪表板 + 知识摘要（`references/report.md`）
