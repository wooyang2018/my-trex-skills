...
1. 遵循 `references/constitution.md` §16 配置解析协议。从 CWD 向上查找 `.env`，回退到 `~/.siyuan-wiki/config`。如果都不存在，你将在步骤 1 中创建 `.env`。

2. 确保 `siyuan-sisyphus` CLI 已安装，然后运行预检：

   ```
   which siyuan-sisyphus || npm install -g siyuan-sisyphus
   siyuan-sisyphus --version
   siyuan-sisyphus notebook list --json
   ```

3. 如果 `--version` 或 `notebook list` 失败，**停止**并引导用户运行 `siyuan-sisyphus init` 配置 profile。不尝试文件系统回退。常见失败原因：API URL 不可达、token 未配置、IP 不在思源白名单。

**捕获每个笔记本的 `id` 和 `name`**——需要 id 来消歧同名笔记本并填充 `SIYUAN_NOTEBOOK_ID`。
...
- **如果没有匹配**，明确询问："笔记本 `<name>` 不存在。需要我创建吗？"如果同意：
  ...
  **硬性阻塞——需要手动提升权限**（根据 `references/constitution.md` §13 校准说明 10）。新创建的笔记本返回权限 `r`（只读）。`notebook create` 成功，但紧接着的 `fs write` 会报权限错误，直到人工打开思源桌面 UI，右键笔记本，授予 `rwd`。没有 CLI 可提升权限。运行 `notebook create` 后，本技能必须：
...
```bash
siyuan-sisyphus fs write --path "/<SIYUAN_NOTEBOOK_NAME>/index" --overwrite --markdown \
...
```bash
siyuan-sisyphus fs write --path "/<SIYUAN_NOTEBOOK_NAME>/log" --overwrite --markdown \
...
```bash
siyuan-sisyphus fs write --path "/<SIYUAN_NOTEBOOK_NAME>/hot" --overwrite --markdown \
