...
```bash
siyuan-sisyphus block append --parent-id <docId> --data-type markdown --data "{{ SELECT * FROM blocks WHERE type='d' ORDER BY updated DESC LIMIT 5 }}"
```

要点：

- 嵌入块**必须独占一行**，与上一段中间留空行。
- 只能渲染 `blocks` 表的行（每行就是一个内容块）。SQL 内可 `JOIN attributes / refs / spans` 等辅助筛选，但 `SELECT` 列表必须是 `blocks` 的列（或 `blocks.*`）。
...
也可直接在 markdown 行尾加 kramdown 属性（注意可能改变块边界，慎用）：

```
段落内容{: name="重要段" custom-progress="30"}
```
...
```bash
siyuan-sisyphus block append --parent-id <docId> --data-type markdown --data "{{{row
左栏
