...
| writing.md 模板段 | 标准 ingest | deep-study 预填来源 |
|---|---|---|
| 一句话定义 | AI 草拟 | 研究综合 |
| `## How It Works` | AI 草拟 | 研究综合（须达 L2 深度） |
| `## Why This Way` | AI 草拟 | 研究综合（须达 L3 深度） |
| `## Examples` | AI 草拟（可空） | 研究综合（必须填，代码示例） |
| `## Common Pitfalls` | 留空给人工 | **deep-study 预填**（研究素材提取真实踩坑） |
| `## Interview Questions` | 留空给人工 | **deep-study 预填**（面试追问链 L1-L4） |
| `## Practice Ideas` | 留空给人工 | **deep-study 预填**（可落地实践方案） |
| `## Flashcards` L1 | AI 生成 | AI 生成（填空题，`==mark==` 挖核心术语，`custom-card-type=cloze`） |
| `## Flashcards` L2 | AI 生成 | AI 生成（问答题，超级块首子块=问题、其余=答案，`custom-card-type=qa`） |
| `## Flashcards` L3 | AI 生成 | AI 生成（单选题，嵌套超级块，正面=题干+选项、背面=答案+解析，`custom-card-type=single-choice`） |
| `## Flashcards` L4 | AI 生成 | AI 生成（多选题，嵌套超级块，正面=题干+任务列表、背面=答案+解析，`custom-card-type=multi-choice`） |
| `## Sources` | 来源引用 | references/ 页面块引用 |
